/// Windows FFI internals — MSVC / MinGW `std::string` layout (32 bytes)
///
/// On Windows with MSVC, the `std::string` layout uses:
///
///  struct basic_string {
///      union {
///          char   _Buf[16];             // SSO buffer (for len <= 15)
///          char*  _Ptr;                 // pointer to heap data
///      };
///      size_t _Mysize;                  // string length
///      size_t _Myres;                   // capacity (allocated - 1)
///  };                                   // total: 32 bytes
///
/// When compiled with MinGW/GCC, the layout matches the GCC __cxx11 ABI
/// (same as Linux).  This module uses the GCC layout since the engine is
...
impl NativeString {
    ...
    /// Safe because both sides share the same UCRT allocator.
...
// ─── Mangled symbol names (GCC / MinGW Itanium ABI on Windows) ──────────────
...
/// Library probe names for Windows.
pub(crate) const LIB_NAMES: &[&str] = &["libNativeClientEngine.dll"];
